<?php  
if(($isPage)) {
	global $previousPage, $previousLink, $nextPage, $nextLink, $kommiku, $theimage,$naviNextChapter, $naviLastChapter, $showAll;
	...
	if(isset($_POST['design'])){
		setcookie('show', $_POST['showAll'] );
		if ( $chapterList->slug !== NULL ) $exturl = $chapterList->slug; else $exturl = '1';
		header('Location: '.HTTP_HOST.$komUrlDash.$seriesUrl.$exturl);
	}
	...
	function img($echo = true,$class = NULL,$title = NULL) {
		global $nextPage, $nextLink, $series, $chapter, $page, $kommiku, $showAll;
		...
		if(  $_COOKIE["show"] == 'true') {
			foreach($kommiku['pageLists'] as $pageNumber){
				$chapterImages .= '<tr class="imageWrapper"><td><img src="'.UPLOAD_URLPATH.$url.$kommiku['pageSource'][$pageNumber].'" /></td></tr>';
			}
			echo ( $chapterImages ); 
			return;
		} else {
